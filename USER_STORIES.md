...
`US-900` 用户可以在设置页配置"任务源文件夹"。配置后，任务中心只读取指定文件夹及其子文件夹中的 Markdown 任务；未配置时读取整个 vault。

`US-901` 任务源文件夹支持配置多个路径。每个路径是一个相对于 vault 根目录的文件夹路径，例如 `Projects` 或 `Daily/Work`。路径可以包含 `/`，但不支持 glob 模式或正则。

`US-902` 配置的文件夹路径递归生效：指定文件夹及其所有子文件夹中的任务都会被读取。例如配置 `Projects` 后，`Projects/SubProject/tasks.md` 中的任务也会进入任务中心。

`US-903` 任务源文件夹只影响读取范围，不影响写入目标。Quick Add 仍写入 Daily Note（按 US-701），禅道同步仍按自身配置写入（按 US-807）。读取与写入分离，用户可以"只看特定文件夹的任务"但"新建任务仍写入 Daily Note"。

`US-904` 配置路径后，不在配置范围内的文件中的任务不进入 TaskCache、不进入 Query 结果、不显示在看板、不计入状态栏统计。CLI `list` / `stats` 也只返回配置范围内的任务。

`US-905` 配置路径可以为空。空配置表示"读取整个 vault"，与未配置行为一致。用户可以通过清空配置恢复默认行为。

`US-906` 配置路径验证：路径必须是 vault 中存在的文件夹。配置无效路径时，设置页提示"文件夹不存在"，但允许保存（用户可能在配置后再创建该文件夹）。

`US-907` 移动文件后，任务源文件夹配置不变。如果文件被移动到配置范围外，其任务从任务中心消失；移动回配置范围内，任务重新出现。配置只基于路径匹配，不追踪单个文件。
...
`US-414` 任务源文件夹配置后，TaskCache、Query、CLI、状态栏统计都只处理配置范围内的文件。读取范围约束是全局性的，不区分 Query Tab、GUI 或 CLI。

`US-415` 任务源文件夹只约束读取，不约束写入。新建任务、禅道同步、拖拽改期等写入操作仍按各自规则（US-701、US-807）写入目标文件，不受此配置限制。

`US-416` 任务源文件夹路径匹配规则：路径必须以配置的文件夹路径开头（前缀匹配），例如配置 `Projects` 后，`Projects/task.md` 和 `Projects/Sub/notes.md` 都匹配，但 `Archive/Projects/old.md` 不匹配。
...
禅道用户是团队中使用禅道做项目管理的开发者 / 项目经理。他们希望：

- 不用在禅道和 Obsidian 之间来回切。
- 在 Obsidian 看板中一眼看到禅道里分配给自己的任务。
- 按截止日期看今日 / 本周 / 本月该做什么。

### 20.3 连接与认证

`US-801` 用户在设置页配置禅道服务器地址（必填）。地址格式为 `https://zentao.example.com` 或 `http://192.168.1.100:80`，不带 `/api.php` 后缀。配置后插件能通过 `requestUrl` 访问该地址。

`US-802` 用户在设置页输入禅道账号和密码。密码用 AES-256-GCM 加密后存入 `data.json`，加密 key 硬编码在插件代码中。用户理解这是"比明文好但不如系统钥匙串"的安全等级。

`US-803` 点击"测试连接"按钮后，插件用填写的地址 + 账号 + 密码调用 `POST /api.php/v1/tokens`。成功返回 token 则提示"连接成功"；失败（网络错误、401、超时）则提示具体原因。

`US-804` 登录成功后，token 被缓存在内存中用于后续 API 调用。token 过期时（API 返回 401），自动用加密存储的密码重新登录；重新登录也失败时，弹出 Notice 提示用户检查设置。

### 20.4 执行选择

`US-805` 禅道 API 要求按执行（execution / sprint）拉取任务。用户在设置页选择"同步模式"：

- **手动选择执行**：用户从下拉列表勾选一个或多个执行。下拉列表来自禅道 API `GET /v1/projects` → `GET /v1/executions?project={id}`。
- **全部指派给我的**：遍历用户参与的所有项目下的所有活跃执行，自动拉取 `status=assignedtome` 的任务。

`US-806` 设置页"刷新执行列表"按钮重新从禅道拉取项目和执行。列表缓存到 `data.json`，避免每次打开设置都请求。

### 20.5 同步目标

`US-807` 用户在设置页选择禅道任务的写入目标：

- **Daily Note**：任务写入当天 Daily Note 文件尾（与 Quick Add 一致）。
- **指定文件**：用户选择 vault 中一个固定 Markdown 文件作为禅道任务收集区。

Daily Notes 不可用时（US-701），Daily Note 模式不可选，提示用户先启用 Daily Notes。

`US-808` 指定文件模式下，用户可以浏览 vault 选择目标文件，或手动输入路径。文件不存在时，同步时自动创建。

### 20.6 看板加载按钮

`US-809` 看板工具栏新增"从禅道加载"按钮（图标：🔄 或自定义）。按钮仅在禅道连接配置完整时显示（地址 + 账号 + 密码均已填写）。

`US-810` 点击"从禅道加载"按钮后，根据当前看板的 Query Tab 类型自动计算截止日期范围：

| Query Tab | 截止日期范围 |
|-----------|-------------|
| 今日 | `deadline == today` |
| 本周 | `deadline ∈ [monday..sunday]` |
| 本月 | `deadline ∈ [month_start..month_end]` |

非时间类 Query Tab（如 TODO、未排期）加载按钮不显示截止日期过滤提示，默认加载所有未关闭任务。

`US-811` 加载过程中按钮显示 loading 状态（旋转图标 + 文案"加载中…"）。加载失败时 Notice 提示错误原因；加载成功时 Notice 提示"已加载 N 条任务"。

`US-812` 加载过程中，用户可以继续操作看板。加载是后台异步的，不阻塞 UI。

### 20.7 任务映射与写入

`US-813` 禅道任务映射为 Obsidian Markdown 任务时，字段对应关系为：

| 禅道字段 | Obsidian 字段 | 说明 |
|---------|-------------|------|
| `name` | 任务标题 | 直接作为 `- [ ]` 后的标题 |
| `deadline` | `📅 YYYY-MM-DD` | 截止日期 |
| `estStarted` | `🛫 YYYY-MM-DD` | 预计开始日期（有值时写入） |
| `pri` | 优先级 emoji | 1→⏫ 2→🔼 3→🔽 4→⏬ |
| `estimate` | `[estimate:: Nh]` | 预计工时（有值时写入） |
| `consumed` | `[actual:: Nh]` | 已消耗工时（有值时写入） |
| `type` | `#zentao-{type}` | 任务类型作为 tag，如 `#zentao-devel` |
| `id` | `[zentao:: {id}]` | 禅道任务 ID，用于去重和溯源 |
| `status`（done/closed） | `- [x]` + `✅ YYYY-MM-DD` | 已完成的任务直接标记完成 |
| `status`（cancel） | `- [-]` + `❌ YYYY-MM-DD` | 已取消的任务标记放弃 |

`US-814` 写入的 Obsidian 任务格式遵守用户设置的 task format flavor（Tasks emoji 或 Dataview inline field），与 Quick Add / CLI 保持一致。

`US-815` 生成的任务行示例：

```md
- [ ] 开发电容光敏监测模块 ⏫ 📅 2025-12-30 🛫 2025-12-25 [estimate:: 1h] [zentao:: 42] #zentao-devel
```

### 20.8 去重与增量同步

`US-816` 去重依据：目标文件中已有 `[zentao:: {id}]` inline field 的任务行，视为已同步。再次加载时：

- 禅道任务未变更（标题、截止日期、状态均相同）：跳过。
- 禅道任务有变更（标题改了、截止日期改了、状态变了）：更新该行。
- 禅道任务已不在拉取结果中（完成了、取消了、不再指派给用户）：不做任何操作。已导入的行保留在 Obsidian 中。

`US-817` 增量同步不删除任何已有任务行。删除只能由用户在源 Markdown 编辑层手动完成。

`US-818` 同步写入走 `app.vault.process`，与现有 writer 一样保证文件级原子性。一次同步可能涉及多行写入，但每个文件的写入是原子的。

### 20.9 移动端适配

`US-819` 移动端"从禅道加载"按钮在工具栏中可见，与桌面端功能完全一致。加载过程是后台异步的，不阻塞触屏操作。

`US-820` 移动端禅道相关设置（服务器地址、账号、密码）在 Obsidian 原生设置页中访问，不需要自定义 full-screen 面板。

### 20.10 安全与隐私

`US-821` 密码使用 `crypto.subtle` AES-256-GCM 加密。加密 key 由插件代码中的固定种子 + vault 路径派生。虽然插件开源导致 key 可提取，但已阻止 `data.json` 明文泄露风险。

`US-822` 禅道地址、账号、密码仅存于用户自己的 vault `data.json` 中。不发送到任何第三方服务。API 请求直接从用户设备发往用户配置的禅道服务器。

`US-823` 用户可以在设置页清除禅道连接配置。清除后加密密码、token 缓存、执行列表缓存一并删除。

### 20.11 错误处理

`US-824` 以下错误场景必须有明确用户反馈：

| 场景 | 反馈 |
|------|------|
| 禅道地址不可达 | "无法连接禅道服务器，请检查地址和网络" |
| 认证失败（401） | "禅道账号或密码错误" |
| Token 过期自动重登失败 | "禅道登录已过期，请重新检查密码" |
| 禅道返回非 200 | "禅道返回错误：{status} {message}" |
| 目标文件写入失败 | "写入失败：{原因}" |
| 未配置禅道连接 | 隐藏加载按钮；设置页显示"禅道连接未配置" |
...
- **多禅道实例**：同时只支持连接一个禅道服务器。
- **禅道自定义字段**：只映射标准字段，不处理禅道自定义扩展字段。
