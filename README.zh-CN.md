...
Zentao Center 是一个 Obsidian 插件，用日 / 周 / 月看板管理 markdown 任务，集成禅道项目管理。
...
- 移动端布局、长按菜单、滑动动作。
- 面向 agent 的 `obsidian zentao-center:*` CLI。

## 安装

从 Obsidian 社区插件安装：

[安装 Zentao Center](https://community.obsidian.md/plugins/zentao-center)
...
2. 从 ribbon、命令面板或 `Ctrl/Cmd+Shift+T` 打开 Zentao Center。
...
Zentao Center 读取 Tasks emoji 和 Dataview inline fields 两种格式：
...
写入格式由 **设置 → Zentao Center → 任务格式风味** 控制。

- **Tasks emoji** 写入 `⏳`、`📅`、`➕`、`✅`、`❌` 等字段。
- **Dataview inline fields** 写入 `[scheduled::]`、`[due::]`、`[created::]`、`[completion::]`、`[cancelled::]` 等字段。

拖拽改期、日期选择、Quick Add、CLI 写操作都会使用这个设置。同一行同一字段如果两种格式都存在，Zentao Center 以 Tasks emoji 为准。
...
Zentao Center 注册到 Obsidian 原生 CLI：
