...
移动端：日期格只显示日期数字和任务数圆点，点击打开该日列表 bottom sheet，不在格内画完整卡片。（US-504）
...
点击普通卡片、今日卡片、搜索结果卡片都打开同一个源 Markdown 编辑层，并定位到 `task.path:task.line`。（US-168 / US-168a）

该入口取代：hover 上下文预览、双击跳源文件、右键打开源文件、卡片标题 inline input。（US-168d / US-161）

体验要求：

- 编辑层覆盖在当前 Task Center 上方，背景保持原 tab / filter / scroll 状态。
...
- 改期：动作 sheet → 日期选择器。
- 放弃：右滑或动作 sheet。
- 嵌套：动作 sheet → 搜索父任务选择器。
- 完成：左滑卡片，阈值 30% 卡宽，1 秒 toast undo。
- 放弃：右滑卡片，阈值 30% 卡宽，1 秒 toast undo。（US-507 / US-508）

Week 移动端是 7 行折叠面板；当前日默认展开；row header 显示 `星期 MM-DD · N tasks · XhYm`。（US-503）

Month 移动端是“日历 + 数字 + 任务数圆点”；点击日期格打开该日列表 bottom sheet。（US-504）

卡片紧凑显示：meta 行合并，子任务默认显 1 层，更多层级用 bottom sheet 展示。长按 ≥ 500ms 且未滚动时弹操作 sheet；单击仍打开源 Markdown 编辑层。（US-505 / US-506）
