...
> 本文只描述 Zentao Center 的界面对象、交互路径、状态反馈与验收边界。
>
...
│  └─ 快捷筛选入口（桌面展开，移动端收进 bottom sheet）
├─ Summary Area
│  └─ 当前 Query 配置的统计指标
├─ View Body
│  ├─ list
...
- Tab 过多：多余 tab 进入“更多”，仍保留排序、badge、快捷键和默认 tab 语义。（US-109q）
...
Toolbar 应用一句可读摘要解释当前条件，例如：

```text
tag:#alpha,#beta · 排期:本周 · 状态:TODO · view:周 · summary:count, sum(actual)
```

空结果时必须说明是“筛选条件无结果”“当前 view 与 filter 组合天然为空”还是“全 vault 无任务”，并给出清空筛选或切换 view 等动作。（US-109b / US-104）
...
2. **View**：类型、section、axis、bucket、排序、空 bucket 显示、未排期 tray。
3. **Summary**：count、sum、ratio、top-N、group-by。

面板内必须能完成当前 tab 的更新、另存为、重命名、复制、隐藏 / 删除、设默认等相关动作，避免跳到设置页完成 CRUD。（US-109n1 / US-109n3）

移动端 Query 编辑使用接近全屏的 bottom sheet，而不是居中弹窗。顶部只放标题、当前 Query 摘要和关闭 affordance；内容按 Filters、View、Summary、DSL、保存与管理分段。Filters 控件在移动端采用两列或单列自适应布局，不能把“标签 / 排期 / 更多时间 / 状态”渲染成占满半屏的四个巨型按钮。View 类型显示为本地化标签：列表、周、月、矩阵。（US-117 / US-117e）
...
搜索输入匹配任务标题关键字；`/` 聚焦搜索框。桌面可直接展示；移动端收进 Query bottom sheet。（US-166 / US-117）
...
Matrix 是用户配置的二维 bucket。（US-103a）
...
Obsidian Mobile 底部工具栏会覆盖 WebView 底部区域。Zentao Center 在真实移动端必须额外预留底部可视与可触达空间，适用范围包括：
...
- 桌面端编辑层覆盖在当前 Zentao Center 上方，背景保持原 tab / filter / scroll 状态。
...
- 移动端从任务详情 / 操作 sheet 的”编辑原文”进入 Obsidian 原生 Markdown 编辑器，不在 Zentao Center 内展示整篇 Markdown 浮层；进入后应定位到任务行，由 Obsidian 原生编辑器处理键盘、安全区和滚动。（US-168g / US-506）
...
| 任务源文件夹 | 空（读取整个 vault） | 支持多路径，递归子文件夹；US-900~907 |
| 恢复默认 tabs | 手动动作 | 只恢复系统预设，不删除用户 tab；US-109l |
| AI skill 安装指引 | 文本命令 | `npx skills add CorrectRoadH/obsidian-zentao-center`；US-215 |

任务源文件夹配置交互：（US-900~907）

- **控件形态**：Obsidian 原生 Setting 的多路径输入。
- **添加路径**：点击"添加文件夹"按钮打开 Obsidian 原生文件夹选择器（`FuzzySuggestModal`），选择 vault 中的文件夹。
- **路径展示**：每个路径显示为一个 chip，右侧有删除按钮。
- **路径验证**：添加时检查文件夹是否存在；不存在时在 chip旁显示警告图标和 tooltip "文件夹不存在"，但仍允许保存。
- **空配置**：没有任何路径时表示"读取整个 vault"，表格说明中标注默认值。
- **递归说明**：设置项下方有说明文本"包含子文件夹中的任务"。
- **写入分离说明**：设置项下方有说明文本"此配置只影响任务读取范围，不影响新建任务的写入位置（Daily Note）"。
- **移动端**：使用 Obsidian 原生 Setting 组件，自动适配移动端布局。
...
`obsidian zentao-center` 是帮助入口，输出可直接复制的命令索引，包含任务读写、Query Tab 管理和 `npx skills add CorrectRoadH/obsidian-zentao-center`。（US-201a / US-215）
...
- `query-run id=<tab-id> view=list|week|month|matrix`：只临时改展示 view，不写回 DSL。
...
1. **不改变主工作流**：禅道是外部数据导入通道，不引入新的核心对象或新的 view 类型。导入后的任务与手动创建的任务完全等价。
2. **手动触发**：用户主动点击按钮才拉取，不自动轮询，不在后台静默写入。
3. **最小侵入**：只在工具栏加一个按钮、设置页加一个分区。不新增 modal / dialog 之外的独立视图。
4. **符合 Obsidian 体验**：设置页使用 Obsidian 原生 `Setting` 组件；加载状态用 `Notice`；错误反馈用 Obsidian 风格。

### 18.2 设置页：禅道连接

设置页新增一个可折叠分区"禅道连接"，包含以下控件：（US-801 ~ US-804）
...
▸ 禅道连接
  ...
  提示：格式 https://zentao.example.com，不带 /api.php

  账号
  [_______________________________]

  密码
  [_______________________________]  (type=password)
  ...
  同步目标
  ( ) Daily Note（写入当天 Daily Note 文件尾）
  ( ) 指定文件  [ 选择文件... ]
...
- 三个必填字段（地址 + 账号 + 密码）均填写后，"测试连接"按钮才可用。
- 点击"测试连接"后按钮变为 loading 态（文案"连接中…"），不可重复点击。
- 成功：`Notice("连接成功")`。
- 失败：`Notice("连接失败：{具体原因}")`，按钮恢复可点击。
- "同步模式"切换到"手动选择执行"时，自动触发一次执行列表刷新（如果已有有效连接）。列表为空时显示"暂无执行，请先在禅道中创建项目迭代"。
- "指定文件"按钮打开 Obsidian 原生文件选择器（`FuzzySuggestModal`），选择 vault 中的 `.md` 文件。
- "清除禅道配置"点击后弹出确认对话框，确认后清除地址、加密密码、token 缓存、执行列表缓存。不删除已同步到 vault 的任务行。
- 密码字段加载时从加密存储解密填充；密码正确时显示为 `••••••••`。

### 18.3 看板工具栏：从禅道加载

工具栏右侧（Quick Add 按钮左侧）新增"从禅道加载"按钮。（US-809）

```text
[今日|本周|本月|TODO|...]  摘要...  [🔄 从禅道加载] [+ Quick Add]
```

按钮行为：

- **显示条件**：禅道配置完整时（地址 + 账号 + 密码均已填写）才显示。未配置时不显示，不占位。
- **按钮图标**：🔄 或使用 Obsidian 内置的 refresh / download 图标。
- **hover 态**：显示 tooltip "从禅道加载任务"。
- **点击后**：
  1. 按钮进入 loading 态：图标旋转 + 文案变为"加载中…"，不可重复点击。
  2. 根据 US-810 自动计算截止日期范围：

  | 当前 Tab | 加载范围提示 |
  |---------|-------------|
  | 今日 | tooltip 或 Notice 不额外提示（隐含截止日期 = 今天） |
  | 本周 | 加载"截止日期在本周的任务" |
  | 本月 | 加载"截止日期在本月的任务" |
  | 其它 | 加载"所有未关闭任务" |

  3. 后台异步执行，不阻塞看板操作。（US-812）
  4. 完成后：
     - 成功：`Notice("已从禅道加载 N 条任务")`，按钮恢复原态，看板自动刷新（走 `cache.on("changed")` 事件链）。
     - 部分成功：`Notice("已加载 N 条任务，M 条失败：{原因}")`。
     - 失败：`Notice("禅道加载失败：{原因}")`，按钮恢复原态。
  5. Token 过期自动重登（US-804）：重登成功则继续；重登失败则 Notice 提示检查设置。

- **移动端**：按钮在 toolbar 中以图标形式展示，尺寸符合 44px 最小触控要求。加载中状态在按钮上体现，不使用独立 Notice 遮挡操作区。（US-819）
...
```text
空闲 → [点击] → 加载中 → [成功] → 空闲（Notice 反馈）
                     ↘ [失败] → 空闲（Notice 反馈）
                     ↘ [Token 过期] → 自动重登 → 加载中
                                           ↘ [重登失败] → 空闲（Notice 提示检查设置）
```

写入过程不弹额外确认。用户点击即执行，结果通过 Notice 反馈。写入是原子的，不会产生半成品。（US-818）

### 18.5 导入后任务的表现

导入的禅道任务与手动创建任务完全等价：

- 在看板中按正常 Query DSL 过滤和渲染。
- 可以拖拽改期、完成、放弃、嵌套。
- 可以在源 Markdown 编辑层中编辑。
- `[zentao:: {id}]` inline field 作为普通 inline field 存在，不特殊渲染。

唯一可识别特征是 `[zentao:: {id}]` inline field 和 `#zentao-{type}` tag，它们在卡片中正常显示为 meta 信息和 tag chip。

### 18.6 错误反馈

所有禅道相关错误通过 Obsidian `Notice` 反馈，不弹独立 modal / dialog。（US-824）

| 场景 | Notice 内容 |
|------|------------|
| 地址不可达 | `无法连接禅道服务器，请检查地址和网络` |
| 认证失败 | `禅道账号或密码错误` |
| Token 过期重登失败 | `禅道登录已过期，请检查设置中的密码` |
| API 非预期响应 | `禅道返回错误：{status} {message}` |
| 目标文件写入失败 | `写入失败：{reason}` |
| 网络超时 | `禅道连接超时，请检查网络` |

Notice 使用 Obsidian 默认 5 秒显示时长。超时设为 15 秒。（US-825）

### 18.7 移动端禅道 UX

移动端禅道功能与桌面完全一致：（US-819 / US-820）

- 设置页使用 Obsidian 原生 `PluginSettingTab`，自动适配移动端布局。
- 加载按钮在 toolbar 中以图标形式展示，44px 最小触控。
- 不额外引入禅道专用的 full-screen 面板或 modal。
- 密码输入使用 `type=password` 的 Obsidian 原生 Setting 输入框。
